...
class Assets
{
    ...
    public static function init()
    {
        ...
        self::$css     = new AssetsBase(self::$BaseUrl, 'css');
        self::$js      = new AssetsBase(self::$BaseUrl, 'js');
    }
     ...
     * @return string
     */
    public static function images( $file, $version = '' )
    {
        ...
        return self::$BaseUrl . Config::get('path.assets') . '/images/' . $file . $version;
    }
     ...
     * @return string
     */
    public static function url( $file = '', $version = '' )
    {
        ...
        if ( !empty( $file ) ) {
            return self::$BaseUrl . Config::get('path.assets') . '/' . $file . $version;
        } else {
            return self::$BaseUrl . Config::get('path.assets') . '/';
        }
    ...
    public function render( $file = '' )
    {
        ...
        if ( $this->type == 'js' ) {
            ...
            if ( $file != '' ) {
                ...
                if ( preg_match(
                    "/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", $asset
                ) ) {
                    $asset_url = $asset;
                } else {
