...
class Kecik 
{
	...
	/**
	 * @var Closure $GroupFunc
	 */
	private $GroupFunc;

	/**
	 * @var array $LibrariesEnabled all loaded libraries
	 */
	...
	public function autoload($class) 
	{
		...
		if (count($ClassArray) > 1) {
			...
			if (count($ClassArray) >= 3) {
				...
				$FileLoad = $MvcPath.'/'.$HmvcPath.strtolower($ClassArray[count($ClassArray)-2]).'s/'.strtolower($ClassArray[count($ClassArray)-1]).'.php';
			} else  {
			    //**                          Controller/Models              Class
				$FileLoad = $MvcPath.'/'.strtolower($ClassArray[0]).'s/'.strtolower($ClassArray[1]).'.php';
			}
